...
pub(crate) async fn subscribe_focus_events(context: Weak<Context>) {
    ...
    unsafe { &*context.as_ptr() }
        ...
        .add_focus_changed_listener(move |x| {
            let event_core = unsafe { &*ctx.as_ptr() }.event_core.clone();
    ...
    unsafe { &*context.as_ptr() }
        .msaa
        .add_on_object_focus_listener(move |src| {
    ...
    unsafe { &*context.as_ptr() }
        .msaa
        .add_on_object_show_listener(move |src| {
    ...
    unsafe { &*context.as_ptr() }
        .jab
        .add_on_focus_gained_listener(move |src| {
            let performer = unsafe { &*ctx.as_ptr() }.performer.clone();
